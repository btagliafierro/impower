...
1. **Turbine behaviour**: The dynamic response of FOWTs under extreme wind and wave loads is not fully understood.
...
**IM-POWER** uses a CFD platform that portrays the first open-source software able to precisely simulate FOWTs, with high accuracy and robustness. The software can be used out of the box, without requiring modification of the source code. The project further provides:
...
The assessment of floating wind farm power output under storm conditions had not been attempted before this project, and no data were publicly available on FOWTs under sea storm conditions. Simulation inputs, geometries and reproducible templates generated by IM-POWER are now openly archived under CC BY 4.0 — see [Outcome](outcome.md).
